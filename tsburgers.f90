...
program tsburgers
  ...
  real (kind=8):: lo_visc, lo_hypvisc, hi_visc, hi_hypvisc
...
contains
  ...
  subroutine read_input_file
    ...
    namelist / parameters / lo_nx, hi_nx, lo_lx, hi_lx, nstep, dt, nwrite,&
         lo_visc, lo_hypvisc, hi_visc, hi_hypvisc, lo_gamma1, hi_gamma1, lo_ikf, hi_ikf, lo_fkinit,&
         hi_fkinit
  ...
  subroutine forcing
    ...
    if (.not.allocated(lo_gamma)) then
  ...
  subroutine time_advance
    ...
    if (.not.allocated(lo_interaction)) then
    ...
    do ik = 1, lo_nk 
       c = 0.5*lo_visc*(lo_kgrid(ik)/maxval(abs(hi_kgrid)))**2+&
           0.5*lo_hypvisc*(lo_kgrid(ik)/maxval(abs(hi_kgrid)))**4
